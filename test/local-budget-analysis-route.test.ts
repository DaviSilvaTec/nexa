...
test('analyzes local budget materials through the HTTP API', async () => {
  const app = createApp({
  ...
  assert.equal(json.type, 'local_budget_analysis_built');
  assert.equal(json.context.customer.contact.id, '999');
  assert.equal(json.materialAnalysis.summary.matchedItems.length, 1);
  assert.equal(json.materialAnalysis.summary.matchedItems[0].id, '1');
